...
1. Your visitor—let’s call her Sally—opens the page with a web form in the browser window. The browser sees the form control elements in the markup and renders them with the appropriate form controls on the page, including two text-entry fields and a Submit button (shown in FIGURE 1)
...
This sample source document contains a form similar to the one shown in FIGURE 1:
...
The method attribute specifies how the information should be sent to the server. Let’s use this data gathered from the sample form in FIGURE 1 as an example.
...
As you will see, the majority of controls are added to a form via the input element. The functionality and appearance of the input element changes based on the value of the type attribute in the tag. In `HTML`5.2, there are twenty-two types of input controls. We’ll take a look at them all.
...
At times, you’ll want your users to be able to enter more than just one line of text. For these instances, use the textarea element, which is replaced by a multiline, scrollable text entry box when displayed by the browser (FIGURE 3).
...
In the following example (FIGURE 7), a datalist suggests several education level options for a text input:
...
#### Drop-down menus
The select element displays as a drop-down menu (also called a pull-down menu) by default when no size is specified or if the size attribute is set to 1. In pull-down menus, only one item may be selected. Here’s an example (shown in FIGURE 12):
```html
...
#### Scrolling menus
To make the menu display as a scrolling list, simply specify the number of lines you’d like to be visible using the size attribute. This example menu has the same options as the previous one, except it has been set to display as a scrolling list that is six lines tall (FIGURE 13):
...
You can use the optgroup element to create conceptual groups of options. The required label attribute provides the heading for the group. FIGURE 14 shows how option groups are rendered in modern browsers.
...
The file selection control makes it possible for users to select a document from the hard drive to be submitted with the form data. We add it to the form by using the input element, with its type set to file. The markup sample here (FIGURE 15) shows a file selection control used for photo submissions:
```html
...
```
The file upload widget varies slightly by browser and operating system, but it is generally a button that allows you to access the file organization system on your computer (FIGURE 15).
...
Creates a combined date/time input control with no time zone informa- tion (YYYY-MM-DDThh:mm:ss).
